...
Sample and measures. The analytic file contains 154 participants: 76 ASD and 78 typically developing (TD) controls. Each row is a participant; columns include serum protein intensities (1,318 protein features in this copy) plus minimal metadata (e.g., Group) and a clinical-severity measure (ADOS Total Score). Raw intensities are strongly right-skewed and vary on a multiplicative scale, so we analyze log-transformed values to stabilize variance and reduce the leverage of extreme highs. Missing protein cells are low (\~1.3%) and are handled consistently with the course scripts; outliers are trimmed by default using a simple rule, with a no-trim variant used for sensitivity checks reported below.
...
The study processed serum from 76 ASD and 78 TD boys (18 months–8 years) using SomaLogic’s SOMAScan 1.3K platform, measuring 1,317 proteins and analyzing 1,125 after QC and log-transformation. Screening proceeded three ways: (1) univariate ASD vs TD comparisons, (2) correlation with ASD severity (ADOS), and (3) random-forest variable importance. Five proteins were consistently top across all three screens and defined the core: MAPK14, IgD, DERM (dermatopontin), EPHB2, and suPAR. Augmenting the core with four proteins that improved AUC—ROR1, GI24 (platelet receptor Gl24), EIF-4H, and ARSB—yielded a 9-protein panel. The authors report AUC = 0.8599 ± 0.0640, sensitivity = 0.835 ± 0.1176, and specificity = 0.8217 ± 0.1178 on held-out evaluation, with duplicate assays showing \<14% variability for these proteins.
...
knitr::include_graphics("../scripts/roc_results_graph.png")
```

**Hard Intersection Results**

```{r, out.width="55%", fig.align="center"}
knitr::include_graphics("../scripts/results_graph.png")
