...
Sample and measures. The analytic file contains 154 participants: 76 ASD and 78 typically developing (TD) controls. Each row is a participant; columns include serum protein intensities (1,317 protein features in this copy) plus minimal metadata (e.g., Group) and a clinical-severity measure (ADOS Total Score). Raw intensities are strongly right-skewed and vary on a multiplicative scale, so we analyze log-transformed values to stabilize variance and reduce the leverage of extreme highs. After cleaning, protein values have \~0% missingness (the small fraction in the raw file came from a blank separator row); outliers are trimmed by default using a simple rule, with a no-trim variant used for sensitivity checks reported below.
