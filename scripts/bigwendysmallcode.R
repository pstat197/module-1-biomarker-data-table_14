...
for (prot_set in 1:20){
  ...
  proteins_s2 <- rf_out$importance %>% 
    ...
    slice_max(MeanDecreaseGini, n = prot_set) %>%
    pull(protein)
  ...
  #class_metrics(
    ...
    #pred
  #)
  auc_result <- roc_auc(test_df, truth = class, pred) %>% pull(.estimate)
  
  
  # print result
  cat("prot_set =", prot_set, "→ ROC AUC:", round(auc_result, 3), "\n")
